...
但愿人长久，千里共婵娟。)";
    ...
    if(fs3.is_open())
    {
        ...
        char buf[13];
        buf[12] = '\0';
        fs3.read(buf,sizeof(buf) -1);
        cout<<"实际读取字节: "<<fs3.gcount()<<endl; //一般只能用于返回read读取的个数，write不行
        cout<<"buf: "<<buf<<endl;

        char bufw[] = "会出来";
        fs3.write(bufw,sizeof(bufw));
        cout<<"是否写入失败: "<<fs3.fail()<<" | bufw: "<<bufw<<endl;
        fs3.close();
    }
    //== 字节读取与写入 ==
...
长风破浪会有时，直挂云帆济沧海。)";
    ...
    if(fd1)
    {
        ...
        cout<<"实际写入字节: "<<size * str_show.size()<<endl;
...
===== 非截断 =====
== show file ==
苏轼：今天写水调歌头给皇帝交差，明月及时会出来。。。算了被贬再说
��又恐琼楼玉宇，高处不胜寒。
起舞弄清影，何似在人间。
转朱阁，低绮户，照无眠。
不应有恨，何事长向别时圆？
...
===== 二进制读写 =====
str_all size: 97| len_file: 97
str_all size: 194| len_file: 97
== show file ==
苏轼：今天写水调歌头给皇帝交差，明月及时会出来。。。算了被贬再说
新内容：苏轼：今天写水调歌头给皇帝交差，明月及时会出来。。。算了被贬再说
苏轼：今天写水调歌头给皇帝交差，明月及时会出来。。。算了被贬再说
== show end ==
