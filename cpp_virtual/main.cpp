...
//! == 多态简介 ==
//! 什么是多态？
//!     简单来说多态就是子类继承父类，并实现父类的虚函数接口，
//!         使用者可以通过创建父类指针指向子类对象，当调用虚函数接口时，
//!         触发子类的具体实现。
//!     从而通过创建多个子类，公用一个父类指针，利用不同子类对象触发同一个接口，
//!         却运行不同代码段，来实现接口统一，功能多变的特性。
//!
//! 为什么使用多态？
...
//!
//! 为什么不使用多态？
//!     多态并不是没有缺点，虚表的延时绑定导致调用多态函数时需要多次加载内存，
//!         C++在高性能场景不会使用多态而是用模板或者回调代替，
